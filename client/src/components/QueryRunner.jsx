...
const LOCAL_API_BASE = '/api/football'
const UPSTREAM_HOST = 'https://api.football-data.org'

function buildUrl(raw, competitionId) {
  let url = typeof raw === 'string' ? raw : (raw && raw.raw) || ''
  url = url.replace(/{{url}}/g, UPSTREAM_HOST)
  ...
  if (url.startsWith(UPSTREAM_HOST)) {
    url = `${LOCAL_API_BASE}${url.slice(UPSTREAM_HOST.length)}` || LOCAL_API_BASE
  }
...
export default function QueryRunner() {
  ...
  const runRequest = async (item, idx) => {
    const raw =
      ...
      ''
    const finalUrl = buildUrl(raw, competitionId)
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <p style={{ marginTop: 0, color: '#555' }}>
        Requests go through the Express server at <code>/api/football</code>. The
        football-data.org token stays on the server.
      </p>
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {responses.map((r) => (
          <div
            key={r.id}
            style={{ border: '1px solid #ddd', padding: 12, borderRadius: 6, background: '#fff' }}
          >
